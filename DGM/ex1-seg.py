...
top_out_dir = '/home/lab1501/Cirp2022DGM/part_seg/gan'
experiment_name = 'latent_gan_with_chamfer_ae'

bneck_size = 128         # Bottleneck-size of the AE
n_pc_points = 2048       # Number of points per model.

class_name = raw_input('"run smoothly, strong stiffness and strong stability" or "strong traction, run smoothly and strong stiffness"').lower()
syn_id = snc_category_to_synth_id()[class_name]
class_dir = osp.join(top_in_dir , syn_id)
all_pc_data = load_all_point_clouds_under_folder(class_dir, n_threads=8, file_ending='.ply', verbose=True)
print 'Shape of DATA =', all_pc_data.point_clouds.shape
pcin=all_pc_data.point_clouds[0:2]
...
parser.add_argument('--model_path', default='train_results_mult/trained_models_ex1/epoch_50.ckpt', help='Model checkpoint path')
...
gpu_to_use = 0
output_dir = os.path.join(BASE_DIR, './test_resultswheel')
...
def predict():
    ...
    with tf.Session(config=config) as sess:
        ...
        for shape_idx in range(1):
            ...
            cur_label_one_hot = np.zeros((2, point_num, 3), dtype=np.float32)
            # cur_label_one_hot[0, cur_gt_label] = 1
            pts = cur_data[0]
            pts=pts[np.newaxis,:,:]
            seg=cur_seg[0,:]
            cur_label_one_hot=pts
        ...
        #np.save("pc_gen1.npy", label_pred_val[0])
