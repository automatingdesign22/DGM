...
def get_model(point_cloud, latent,input_label, is_training, cat_num, part_num, \
    ...
    net_transformed = tf.expand_dims(squeezed_out3, [2])

    out4 = tf_util.conv2d(net_transformed, 256, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv4', bn_decay=bn_decay)
    out5 = tf_util.conv2d(out4, 128, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv5', bn_decay=bn_decay)
    out_max = tf_util.max_pool2d(out5, [num_point,1], padding='VALID', scope='maxpool')
    out_max = tf.nn.sigmoid(out_max)

    # classification network
    net = tf.reshape(tf.convert_to_tensor(latent), [batch_size, -1])
    ...
    outsize=np.prod(point_cloud.get_shape().as_list()[1:])
    net = tf_util.fully_connected(net, outsize, activation_fn=None, scope='cla/fc3')
...
def get_loss(l_pred, seg_pred, label, seg, weight, end_points):
    ...
    # print(l_pred.shape,label.shape)
    cost_p1_p2, _, cost_p2_p1, _ = nn_distance(l_pred, label)
    label_loss = tf.reduce_mean(cost_p1_p2) + tf.reduce_mean(cost_p2_p1)
    per_instance_label_loss = label_loss
