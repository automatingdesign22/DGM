...
pcin=all_pc_data.point_clouds

cur_seg = np.zeros([pcin.shape[0],2048],dtype=int)
cur_data = all_pc_data.point_clouds
cur_labels = cur_data

for i in range(len(pcin)) :
    for j in range(2048):
        if np.square(pcin[i,j,0])+np.square(pcin[i,j,2])<np.square(0.24):
            cur_seg[i,j] = int(0)
        elif np.square(pcin[i,j,0])+np.square(pcin[i,j,2])>np.square(0.26):
            cur_seg[i,j] = int(1)
        else:
            cur_seg[i,j] = int(2)
...
parser.add_argument('--output_dir', type=str, default='train_results_mult',
...
MODEL_STORAGE_PATH = os.path.join(output_dir, 'trained_models_ex1_1')
...
def placeholder_inputs():
    ...
    labels_ph = tf.placeholder(tf.float32, shape=(batch_size, point_num, 3))
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(FLAGS.gpu)):
            ...
            loss, label_loss, per_instance_label_loss, seg_loss, per_instance_seg_loss, per_instance_seg_pred_res \
                = model.get_loss(labels_pred, seg_pred, labels_ph, seg_ph, 0.0001, end_points)
        ...
        def train_one_epoch(train_file_idx, epoch_num):
            ...
            for i in range(1):
                ...
                # cur_data, cur_labels, order = provider.shuffle_data(cur_data, cur_data)
                # cur_seg = cur_seg[order, ...]

                # cur_labels_one_hot = convert_label_to_one_hot(cur_labels)
                cur_labels_one_hot = cur_data
        ...
        def eval_one_epoch(epoch_num):
            ...
            for i in range(1):
                ...
                # cur_data, cur_labels1, cur_seg = provider.loadDataFile_with_seg(cur_test_filename)
                cur_labels = cur_data
